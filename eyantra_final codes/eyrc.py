...
```
#* Team Id : eYRC#PB#1108

#* Author List : Mohammed Arfan ,Swagath B

#* Filename: Planter_bot_main_code

#* Theme: eYRC PLANTER BOT


#* Functions: forward,turnleft,turnright,turn_softleft,turn_softright,turn_simpleleft,stop,select_rgb_black,apply_smoothing,
#* detect_edges,filter_region,select_region,select_region1,select_region2,select_region3,select_regionB
#* get_contour,get_centroid,draw_output,compute_pwm,back_end,detect_shape,make_overlay,blink_end,main.


#* Global Variables: rowsp,colsp,font,GREEN,RED,BLUE,Motor1A,Motor1B,Motor1E,Motor2A,Motor2B,Motor2E
#* left,right,img0.
...
GREEN = 26              #GREEN PIN RGB LED
RED = 24                #RED PIN OF RGB LED
BLUE=22                 #BLUE PIN OF RGB LED
...
#blinking RGB LED IN SEQUENCE TO INDICATE BEGINING OF THE RUN
...
#Reading the background image containing zones
...
#* Input: speed
#* Output: none
#* Logic: moves the bot forward by setting PWM
...
#* Input: centroid of the contour
#* Output: none
#* Logic: moves the bot to the left proportional to the deference between centroid
#* and the frame centre by setting PWM
#* Example Call: turn_left(30)
#```
def turn_left(cx):
...
#```
#* Function Name: turn_softleft
#* Input: centroid of the contour
#* Output: none
#* Logic: moves the bot to the left proportional to the deference between centroid
#* and the frame centre by setting PWM
#* Example Call: turn_softleft(30)
#```
...
#```
#* Function Name: turn_simpleleft
#* Input: speed
#* Output: none
#* Logic: moves the bot to the left 
#* Example Call: turn_simpleleft(30)
#```
...
#```
#* Function Name: turn_softright
#* Input: centroid of the contour
#* Output: none
#* Logic: moves the bot to the right proportional to the deference between centroid
#* and the frame centre by setting PWM
#* Example Call: turn_softright(100)
#```
...
#```
#* Function Name: apply_smoothing
#* Input: image
#* Output: smoothened image
#* Logic: gaussian bluring
#* Example Call: apply_smoothing(image)
#```
...
#```
#* Function Name: filter_region
#* Input: image,vertices
#* Output: region of interest
#* Logic: masking and bitwise_and
#* Example Call: filter_region(image,vertices)
#```
...
#* Input: binary image and color image
#* Output: region of intrest
#* Logic: defining the vertices of the region of intrest
#* Example Call: select_region(image,image1)
#```
...
def select_region3(image,image1):
    ...
    return filter_region(image,vertices)
#####################################################################################
#REGIONS FOR CONTOUR DETECTION IN THE INVERTED PLANE
#####################################################################################
def select_regionIP1(image,image1):
    rows,cols =image.shape[:2]
    bottom_left    = [cols*0.2,rows*1]           #DECRESING THE AREA OF SIGHT SO THAT IT SEES INLY THE WHITE LINE BETWEEN TWO BLACK PATCHES
...
#```
#* Function Name: get_contour
#* Input: image
#* Output: contours
#* Logic: cv2.findContours
#* Example Call: get_contour(image)
#```
...
#* Logic: moments
#* Example Call: get_centroid(contours)
#```
...
def get_centroid(contours):
        ...
        return cx,cy
#```
#* Function Name: draw_output
#* Input: image,contour and centroid
#* Output: detected contours are drawn on the image
#* Logic: draw functions of opencv
#* Example Call: draw_output(image,contours,30,40)
#```
...
#```
#* Function Name: compute_pwm
#* Input: centroid(row),main color frame,scaling factors for pwm 
#* Output: none
#* Logic: proportionally updating the pwm of the motors depending on the deviation of centroid from the centre
#* Example Call: compute_pwm(30,image,0.003,0.0003,1)
#```
...
#####################################################################################
#```
#* Function Name: back_end
#* Input: image
#* Output: shape of color marker,color of CM,number of CM
#* Logic:  hsv colorspace thresholding and perimeter of the contour 
#* Example Call: shape,color,number=back_end(image)
#```
...
#* Output: shape of color marker,color of CM,number of CM
#* Logic:  hsv colorspace thresholding and perimeter of the contour 
#* Example Call: shape,color,number=detect_shape()
#```
def detect_shape():
...
#```
#* Function Name: make_overlay
#* Input: image,shape,color,numbers,zone
#* Output: overlayed image
#* Logic:  bitwise_and,mask
#* Example Call: overlayed_image=make_overlay(image,shape,color,number,zone)
#```
...
#```
#* Function Name: blink_end
#* Input: color,numbers
#* Output: none
#* Logic:  blinking that particular color that many times
#* Example Call: blink_end(red,1)
#```
